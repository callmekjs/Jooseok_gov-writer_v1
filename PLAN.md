...
| G2 | 포트 | 백엔드 **8011**, 프론트 **5174**. 8000 금지 |
| G3 | 모델 허용목록 | Task 3의 `catalog.py` 6개가 전부. **목록에 없는 id를 지어내지 말 것** (`gpt-6-astra`, `claude-opus-4-1` 금지) |
...
| G12 | 키 보관 | LLM 키는 서버 환경변수(기본, 서버가 직접 호출)나 요청 헤더(선택, 사용자가 대신 실어 보낼 때)로만. 서버 디스크·DB·로그 저장 금지 |
